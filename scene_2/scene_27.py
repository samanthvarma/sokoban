...
def solve_ice_with_boxes():
    ...
    box_pos = tuple(map(int, np.argwhere(grid == 4)[0]))  # Only consider the first box like in the single-box solver
    ...
    for t in range(max_steps):
        ...
        solver.add(PbEq([(box_vars[(t+1, r, c)], 1) for r in range(H) for c in range(W) if not is_obstacle(r, c)], 1))
        for r in range(H):
            for c in range(W):
...
if __name__ == "__main__":
    grid, robot_path, box_path, goal_pos = solve_ice_with_boxes()
    if grid is not None:
